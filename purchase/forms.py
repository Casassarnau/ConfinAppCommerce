...
class FilterForm(forms.Form):
    category = SelectCategoryField(queryset=models.PrimaryCategory.objects.all(),
                                   placeholder="Buscar categoria...", required=False, is_loading=False,
                                   title="Filtra per categoria:")
    service = SelectCategoryField(queryset=models.Service.objects.all(),
                                  placeholder="Buscar servei...", required=False, is_loading=False,
    ...
    time = forms.TimeField(required=True, label='A quina hora vols anar a comprar?',
                           ...
                           widget=TimeInputCool())

    location = forms.CharField(required=True, label='Localització',
                               widget=forms.TextInput(attrs={'placeholder': 'Clica\'m!', 'readonly': 'True'}))
